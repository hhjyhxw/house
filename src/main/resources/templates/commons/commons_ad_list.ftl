...
<html>
    ...
    <body>
        ...
        <fieldset class="layui-elem-field">
             <legend>查询条件</legend>
            <form action="${request.contextPath}/backpage/commonsAd/list" id="formId" style="padding-left:33px;">
                 <ul class="findTool">
                     <input type="hidden" id="pageNum" name="pageNum" value="${page.pageNum}">
                     <li> 名称： <input type="text" name="adName" id="adName"  value="${adName!''}" placeholder="" autocomplete="off" ></li>
                     </li>
                ...
                <table class="site-table table-hover">
                    ...
                    <#list page.list as t>
                        <tr>
                         ...
                         <td>${t.adName!''}</td>
                         <td>
                            <img src="${request.contextPath}${t.adImgurl!''}" />
                           </td>
                         <td>${t.addJumpurl!''}</td>
                         <td>
                               <#if t.status?? && t.status=='0'>停用</#if>
                               <#if t.status?? && t.status=='1'>启用</#if>
                         </td>
                         <td>${(t.createTime?string("yyyy-MM-dd HH:mm:ss"))!}</td>
                         <td>${t.createOperator!''}</td>
                         <td>${(t.modifyTime?string("yyyy-MM-dd HH:mm:ss"))!}</td>
                         <td>${t.modifyOperator!''}</td>
                         <td>${(t.sortNum?c)!}</td>
                            <th>
        ...
        $('#toAdd').on('click', function() {
            window.location.href = "${request.contextPath}/backpage/commonsAd/toinput"
        });
        //跳转修改
        function toEdit(id){
            window.location.href = "${request.contextPath}/backpage/commonsAd/toinput?id="+id;
        }
        //删除
        ...
        function delById(id){
            if(confirm("删除了不能恢复,确定要删除？")){
                var url = '${request.contextPath}/backpage/commonsAd/del';
                $.post(url, {
                    'id':id
                }, function(data) {
                     if("0000"==data.code){
                        layer.msg("删除成功");
                        window.location.href = "${request.contextPath}/backpage/commonsAd/list";
                    }else{
                        layer.msg("删除失败");
                    }
                });
